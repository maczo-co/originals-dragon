...
You can verify any **Dragon Tower** bet in three ways. All of them run **the same math the server used** —
nothing is trusted, everything is recomputed.

## What you need

From your Dragon Tower bet history, each bet shows:
...
[`paytable.json`](https://github.com/maczo-co/originals-dragon/blob/main/paytable.json); the verifier
...
```
https://verify.maczo.co/originals/?game=dragon&server=<revealed>&commit=<hash>&client=<you>&nonce=<n>&params=<url-encoded JSON>
```

## Method 2 — Browser, manual

1. Open https://verify.maczo.co/originals/?game=dragon (works fully offline; view source).
...
```bash
git clone https://github.com/maczo-co/originals-dragon
cd originals-dragon
